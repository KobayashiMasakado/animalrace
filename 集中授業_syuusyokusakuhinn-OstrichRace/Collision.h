...
//--------------------------------------//
	...
	static bool HitCheck_Sphere2Capsule(const Sphere& sphere, const Capsule& capsule)
	{
		...
		float dist2 = SqDistPointSegment(capsule.start, capsule.end, sphere.center);
	...
	static bool HitCheck_Capsule2Capsule(const Capsule& capsule1, const Capsule& capsule2)
	{
		...
		float dist2 = ClosestPtSegmentSegment(capsule1.start, capsule1.end, capsule2.start, capsule2.end, s, t, c1, c2);
	...
	{
		if (fabsf(box1.center.x - box2.center.x) > (box1.r.x + box2.r.x)) return false;
		if (fabsf(box1.center.y - box2.center.y) > (box1.r.y + box2.r.y)) return false;
		if (fabsf(box1.center.z - box2.center.z) > (box1.r.z + box2.r.z)) return false;
		return true;
	}
...
//------------------------------//
	...
	static float SqDistPointBox(const DirectX::SimpleMath::Vector3& p, const Box& b)
	{
		...
		float min[3] = { b.center.x - b.r.x, b.center.y - b.r.y, b.center.z - b.r.z, };
		float max[3] = { b.center.x + b.r.x, b.center.y + b.r.y, b.center.z + b.r.z, };
