...
void MyEffect::Create2(DX::DeviceResources* deviceResources, ID3D11ShaderResourceView* texture)
{
	m_deviceResources = deviceResources;
	auto device = m_deviceResources->GetD3DDevice();
...
void MyEffect::Update2(DX::StepTimer timer)
{
	float time = float(m_timer.GetElapsedSeconds());
	m_timer = timer;
	if (m_startVelocity.Length() > 0.02f) {
...
void MyEffect::Render2()
{

	m_world = Matrix::CreateBillboard(m_position, m_camera, Vector3::UnitY);
	Draw2(m_world, m_view, m_proj);
...
void MyEffect::SetRenderState2(DirectX::SimpleMath::Vector3 camera, DirectX::SimpleMath::Matrix view, DirectX::SimpleMath::Matrix proj)
{
	m_camera = camera;
	m_view = view;
	m_proj = proj;
...
void MyEffect::Draw2(DirectX::SimpleMath::Matrix world, DirectX::SimpleMath::Matrix view, DirectX::SimpleMath::Matrix proj)
{
	auto context = m_deviceResources->GetD3DDeviceContext();
	// 頂点情報（ここは自分で設定してください。）
	VertexPositionColorTexture vertex[4] =
	{
		VertexPositionColorTexture(Vector3(1.002f, 0.5f, 0.0f),Vector4::One, Vector2(0.0f, 0.0f)),
		VertexPositionColorTexture(Vector3(-1.002f, 0.5f, 0.0f),Vector4::One, Vector2(1.0f, 0.0f)),
		VertexPositionColorTexture(Vector3(-1.002f, -0.5f, 0.0f),Vector4::One, Vector2(1.0f, 1.0f)),
		VertexPositionColorTexture(Vector3(1.002f,-0.5f, 0.0f),Vector4::One, Vector2(0.0f, 1.0f)),
	};
	...
	context->PSSetShaderResources(0, 1, &m_texture2);
...
void MyEffect::Draw(DirectX::SimpleMath::Matrix world, DirectX::SimpleMath::Matrix view, DirectX::SimpleMath::Matrix proj)
{
	...
	VertexPositionColorTexture vertex[4] =
	{
		VertexPositionColorTexture(Vector3(0.01f, 0.5f, 0.0f),Vector4::One, Vector2(0.0f, 0.0f)),
		VertexPositionColorTexture(Vector3(-0.01f, 0.5f, 0.0f),Vector4::One, Vector2(1.0f, 0.0f)),
		VertexPositionColorTexture(Vector3(-0.01f, -0.5f, 0.0f),Vector4::One, Vector2(1.0f, 1.0f)),
		VertexPositionColorTexture(Vector3(0.01f,-0.5f, 0.0f),Vector4::One, Vector2(0.0f, 1.0f)),
	};
